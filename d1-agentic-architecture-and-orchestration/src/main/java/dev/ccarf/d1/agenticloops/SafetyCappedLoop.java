...
/**
 * Exercise 1.1.6
 * Add a safety iteration cap of 20 as a maximum bound (not the primary
 * stopping mechanism) and log a warning if it triggers. The loop's actual
 * stopping mechanism is still stop_reason (see the tool_use check below) -
 * MAX_ITERATIONS only guards against a runaway loop and should never be hit
 * in normal operation.
 */
public class SafetyCappedLoop {
  ...
  static String runLoop(AnthropicClient client, String userMessage) {
    ...
    System.err.println("WARNING: safety cap of " + MAX_ITERATIONS + " tool-use iterations hit "
        + "without reaching a terminal stop_reason - this indicates the model is stuck in a "
        + "tool-use loop and should not happen in normal operation.");
